(function () {
  ...
  const dayMeta = {
    en: {
      ...
      startDesc:
        "Day 1 begins today. The plan runs 30 days, in a repeating 6-day cycle.",
      importBtn: "Import a saved plan",
      ...
      catchupText: (list) =>
        `You have ${list.length} day(s) waiting: ${list.map((d) => "Day " + d.n + " (" + d.title + ")").join(", ")}. Tap them below whenever you can.`,
      today: "Today",
      alldays: "All 30 days",
      ...
      problemPlaceholder:
        "e.g. no real logo, inconsistent colors, phone-camera photos...",
      noteLabel: "Note (optional)",
      notePlaceholder: "Anything worth remembering about today",
      editWindowNote:
        "This day is view-only now — editing stays open for the last 3 days only.",
      dayAdvanceNote:
        "The day number moves forward with the real calendar, one day at a time — finishing early doesn't skip ahead to tomorrow.",
      yourprospect: "Yesterday's prospect",
      reconnectwho: "Share this person's post today",
      markDone: "Mark today done",
      doneLabel: "Done ✓",
      sigNone: "No signal",
      sigReply: "Got a reply",
      sigConvo: "New conversation",
      resetBtn: "Reset everything (fresh start)",
      restartBtn: "Start over",
      startDatePrefix: "Today is",
      bigGapText: (n) =>
        `${n} days missed in a row — start fresh if you'd like`,
      notStartedText:
        "No day logged yet — reset the date if this was just a test",
      footer: "Raykom Fil Projet — precision, honesty, proof.",
      poslabel: "Your edge",
      postext:
        "Precision from geomatics. Honesty I won't compromise on. Every brand gets a precisely built, honestly built system — backed by real results since 2023.",
      confirmReset: "Are you sure? This wipes all progress and starts over.",
      sprintDone:
        "30 days complete. Review the results and decide the next cycle.",
      copyBtn: "COPY",
      copied: "COPIED",
      tasks: [
        {
          title: "Scout + Engage",
          desc: "Find one new small jewelry/accessories brand (under 5K followers, weak visual identity, still active). Save the handle below. Then leave a real comment — not just a like — on 3 accounts: this prospect, a fellow designer, and a past client.",
        },
        {
          title: "Outreach",
          desc: "Message the prospect you found yesterday. Use the template below, personalize the bracketed part, and send.",
        },
        {
          title: "Targeted Engagement",
          desc: "Comment for real on 3 more jewelry/accessories accounts — new ones, not the same as before. This is what fixes who actually sees you, not just how many.",
        },
        { title: "Content", desc: null },
        { title: "Reconnect (no ask)", desc: null },
        {
          title: "Reply + Review",
          desc: "Clear any replies or comments waiting for you. Quick check: how many days did you complete this week? Any replies from outreach? Any new signals?",
        },
      ],
      contentTypes: [
    ...
    ar: {
      ...
      catchupText: (list) =>
        `عندك ${list.length} يوم مستني: ${list.map((d) => "يوم " + d.n + " (" + d.title + ")").join("، ")}. كبس عليهم تحت وقتما تحب.`,
      today: "اليوم",
      alldays: "الـ30 يوم",
      monthgoal: "هدف الشهر",
      newclients: "Clients جداد",
      prospectlabel: "اسم البراند (احفظو — تحتاجو غدوة)",
      problemLabel: "شنوة الضعف فـ الهوية البصرية متاعهم؟",
      problemPlaceholder:
        "مثال: بلا لوقو حقيقي، ألوان مش متناسقة، صور موبايل...",
      noteLabel: "ملاحظة (اختياري)",
      notePlaceholder: "أي حاجة تحب تتفكرها على اليوم",
      editWindowNote:
        "هاذا اليوم توا للمشاهدة بس — التعديل يبقى متاح لآخر 3 أيام بس.",
      dayAdvanceNote:
        "رقم اليوم يتقدم مع التاريخ الحقيقي، يوم بيوم — كي تكمل بدري ما يقفزش لغدوة.",
      yourprospect: "البراند اللي لقيتو أمس",
      reconnectwho: "شارك بوست هاذا الشخص اليوم",
      markDone: "علّم اليوم كتم",
      doneLabel: "تم ✓",
      sigNone: "بلا رد",
      sigReply: "جاني رد",
      sigConvo: "محادثة جديدة",
      ...
      notStartedText:
        "لسا ما علمت حتى يوم — تقدر تصفي التاريخ إذا كانت بس تجربة",
      footer: "Raykom Fil Projet — دقة، صدق، دليل.",
      poslabel: "الميزة الحقيقية متاعك",
      postext:
        "الدقة يلي جتني من الجيوماتيك، والصدق يلي ما نتنازلش عليه — هوما يلي يميزوني. كل براند نخدم معاه، يلقى نظام مضبوط بالتفصيل، مبني على الحقيقة مش المبالغة — ونتائج حقيقية تشهد بيه من 2023 لتوا.",
      ...
      tasks: [
        {
          title: "Scout + تفاعل",
          desc: "تلقى براند مجوهرات/إكسسوارات صغير جديد (تحت 5K follower، هوية بصرية ضعيفة، نشيط). تسجل اسمو تحت. بعدها تعلق بجدية (ماشي لايك بس) على 3 حسابات: هاذا البراند، designer، وعميل قديم.",
        },
        {
          title: "Outreach",
          desc: "تبعث رسالة للبراند اللي لقيتو أمس. استعمل القالب تحت، بدل الجزء بين قوسين، وابعث.",
        },
        {
          title: "تفاعل مستهدف",
          desc: "تعلق بجدية على 3 حسابات مجوهرات/إكسسوارات إضافية — جداد، ماشي نفس اللي قبل. هذا يصلح شكون يشوفك فعليًا، ماشي بس كم واحد.",
        },
        { title: "Content", desc: null },
        { title: "Reconnect (بلا طلب)", desc: null },
        {
          title: "رد + مراجعة",
          desc: "تصفي كل رد/تعليق وصلك. مراجعة سريعة: كم يوم عملت هالأسبوع؟ فما رد من الـ outreach؟ فما تفاعل جديد؟",
        },
      ],
      contentTypes: [
  ...
  function load() {
    try {
      ...
      return { startDate: null, days: {}, clients: 0 };
    }
  }
  function save(s) {
    try {
  ...
  function normalizeImportedState(value) {
    const imported = value && value.state ? value.state : value;
    if (
      !imported ||
      (imported.startDate !== null && typeof imported.startDate !== "string") ||
    ...
    Object.keys(imported.days).forEach((key) => {
      ...
      if (
        ...
        days[n] = {
          ...
          signal: ["none", "reply", "convo"].includes(day.signal)
            ? day.signal
            : null,
          prospect: typeof day.prospect === "string" ? day.prospect : "",
    ...
    return {
      ...
      clients: Math.max(
        ...
        Math.min(
          2,
          Number.isFinite(Number(imported.clients))
            ? Number(imported.clients)
            : 0,
        ),
      ),
    };
  ...
  function importPlan(file) {
    ...
    reader.onload = function () {
      try {
        ...
        if (!imported) throw new Error("invalid plan");
        state = imported;
        save(state);
  ...
  function getDayEntry(n) {
    if (!state.days[n])
      state.days[n] = { done: false, signal: null, prospect: "" };
    return state.days[n];
  }
  ...
  function getTaskInfo(n, lang) {
    ...
    let title = base.title,
      desc = base.desc;
    if (ci === 3) {
      desc = t.contentTypes[cyc % t.contentTypes.length];
    }
    if (ci === 4) {
  ...
  function renderStart() {
    ...
    document.getElementById("btnImportStart").textContent = t.importBtn;
    document.getElementById("btnExport").textContent = t.exportBtn;
    document.getElementById("txt-or").textContent = t.or;
    const today = new Date();
  ...
  function renderStats() {
    ...
    let streak = 0;
    for (let i = n; i >= 1; i--) {
      if (state.days[i] && state.days[i].done) streak++;
      else break;
    }
    document.getElementById("statStreak").textContent = streak;
    let signals = 0;
    Object.values(state.days).forEach((d) => {
      if (d.signal === "reply" || d.signal === "convo") signals++;
    });
  ...
  function renderCatchup() {
    ...
    if (missed.length) {
      ...
      document.getElementById("catchupText").textContent =
        t.catchupText(missed);
    } else {
  ...
  function findLastProspect(beforeDay) {
    for (let i = beforeDay - 1; i >= 1; i--) {
      if (state.days[i] && state.days[i].prospect)
        return state.days[i].prospect;
    }
    return "";
  }
  ...
  function renderToday() {
    ...
    document.getElementById("todayDayLabel").textContent =
      t.dayLabel(n) + " — " + info.title;
    document.getElementById("todayTaskTitle").textContent = info.title;
    document.getElementById("todayTaskDesc").textContent = info.desc;

    document.getElementById("scoutField").style.display =
      info.ci === 0 ? "block" : "none";
    document.getElementById("outreachField").style.display =
      info.ci === 1 ? "block" : "none";
    document.getElementById("reconnectField").style.display =
      info.ci === 4 ? "block" : "none";

    if (info.ci === 0) {
      document.getElementById("txt-prospectlabel").textContent =
        t.prospectlabel;
    ...
    if (info.ci === 1) {
      ...
      document.getElementById("prospectDisplay").value =
        findLastProspect(n) || "—";
    ...
    if (info.ci === 4) {
      ...
      const cyc = cycleNumber(n);
      document.getElementById("reconnectDisplay").value =
        clientNames[cyc % clientNames.length];
    }
    ...
    ["none", "reply", "convo"].forEach((s) => {
      const el = document.getElementById(
        "sig" + s.charAt(0).toUpperCase() + s.slice(1),
      );
  ...
  function renderDayList() {
    ...
    for (let i = 1; i <= TOTAL_DAYS; i++) {
      ...
      row.className =
        ...
        (isExpanded ? " expanded" : "");
      const sigIcon =
        entry && entry.signal === "reply"
          ? "\ud83d\udcac"
          : entry && entry.signal === "convo"
            ? "\ud83d\udd25"
            : "";
      ...
      if (isExpanded) {
        ...
        if (info.ci === 0) {
          extraFieldHtml =
            '<div class="dd-field"><label>' +
            t.prospectlabel +
            '</label><input type="text" ' +
            disabledAttr +
            ' data-note-type="prospect" data-day="' +
            i +
            '" value="' +
            (e.prospect || "").replace(/"/g, "&quot;") +
            '" placeholder="@handle"></div>' +
            '<div class="dd-field"><label>' +
            t.problemLabel +
            '</label><input type="text" ' +
            disabledAttr +
            ' data-note-type="note" data-day="' +
            i +
            '" value="' +
            (e.note || "").replace(/"/g, "&quot;") +
            '" placeholder="' +
        ...
        detail.innerHTML =
          ...
          "</div>" +
          (editWindowLocked
            ? '<div class="dd-locked-note">' + t.editWindowNote + "</div>"
            : "") +
          extraFieldHtml;
  ...
  function renderGoal() {
    ...
    document
      .querySelectorAll("#goalSlots .slot")
      .forEach((s, i) => s.classList.toggle("filled", i < state.clients));
  }
  ...
  function applyLang(lang) {
    ...
    document.getElementById("txt-h1").innerHTML =
      lang === "ar"
        ? '30 يوم.<br><span class="hl">نظام واحد.</span>'
        : '30 Days.<br><span class="hl">One system.</span>';
    document.getElementById("txt-sub").textContent =
      lang === "ar"
  ...
  function renderAll() {
    ...
    document.getElementById("completionPanel").style.display = finished
      ? "block"
      : "none";

    const bigGapEl = document.getElementById("bigGapNotice");
    bigGapEl.style.display = "block";
    if (typeof restartSubtleCtrl !== "undefined")
      restartSubtleCtrl.refreshLabel();
  }
  ...
  ["none", "reply", "convo"].forEach((s) => {
    document
      .getElementById("sig" + s.charAt(0).toUpperCase() + s.slice(1))
      .addEventListener("click", () => {
        const n = currentDayNumber();
        const entry = getDayEntry(n);
        entry.signal = s;
  ...
  function wireResetButton(id, defaultTextFn) {
    ...
    return {
      refreshLabel: () => {
        if (!pending) {
          el.textContent = defaultTextFn(dayMeta[currentLang]);
        }
      },
    };
  }
  ...
  const restartSubtleCtrl = wireResetButton(
    "btnRestartSubtle",
    (t) => t.resetBtn,
  );
